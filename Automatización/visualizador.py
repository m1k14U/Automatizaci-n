...
def aceptar():
    ...
    for ki in range(kd):
        if kmin >= 3:
            for kj in range(3):
                ...
                bomba = Canvas(win2, height=100, width=200)
                ...
                tube1_1 = bomba.create_rectangle(105, 55, 110, 95, fill="white", outline="black")
                tube2_1 = bomba.create_rectangle(130, 55, 135, 95, fill="white", outline="black")
    ...
    pumpbtn.grid(row=1, column=7, columnspan=1, padx=10, pady=5)
...
def win2aceptar():
    ...
    win5btn = Button(win4, text="Siguiente", command=win5f)
...
def win5f():
    ...
    if msgwin5 == "yes":
        ...
        def cmdwin6lg():
            ...
            listacarga = [["Acetona", 18], ["NaOH", 18], ["Benzaldehído", 17]]
...
def oopfun():
    ...
    def bomba(ir, jr):
        ...
        GPIO.setup(listadopin,GPIO.OUT)
        print(f"Bomba {listabomba[jr]}  encendida")
        variable=listadopin[jr]
        GPIO.output(variable,0)
        reloj=int(valoresml[ir][jr])
        print(type(reloj))
        time.sleep(reloj)
        GPIO.output(variable,1)
